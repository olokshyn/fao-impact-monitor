...
flowchart TB
  ...
  subgraph exec ["Execution (per Run)"]
    ...
    Doc["Document<br/>stage_results keyed by Workflow.name.WorkflowNode.name"]

    Exec -->|"1. claim SCHEDULED → RUNNING"| Task1
    Exec -->|"2. resolve Stage via<br/>workflow_id + workflow_node_name"| Stage
    Doc -->|"prior StageResults<br/>keyed by namespaced node name"| Stage
    Stage -->|"3. run → StageResult;<br/>atomic Document updates under node name"| SR
    SR -->|"4. stored on Task by Executor"| Task1
    Stage -->|"3b. Document.stage_results[Workflow.name.Node.name]<br/>+ other selected fields"| Doc
...
1. `Task` represents a unit of work in the system. Newly constructed tasks start in `CREATED`. After `Workflow.submit`, they become `SCHEDULED` and point at a concrete `WorkflowNode` via `workflow_id` and `workflow_node_name` (namespaced as `f"{Workflow.name}.{WorkflowNode.name}"`—not only at a `stage_name`, because the same `Stage` may appear more than once in a `Workflow`).
1. `Stage` is a worker that performs this unit of work. It must **not** create `Task`s: it does not know which `WorkflowNode`s come next. It only returns a `StageResult` with enough information for routing. While running, it can read prior `StageResult`s from the `Document` (keyed by namespaced node id `f"{Workflow.name}.{WorkflowNode.name}"`), and it is responsible for atomically writing its own `StageResult` (and any other selected document fields) under that same namespaced key—never by overwriting the whole document.
...
1. `Document` is the durable product of the pipeline. Each finished `Stage` stores its `StageResult` on the `Document` under the namespaced key `f"{Workflow.name}.{WorkflowNode.name}"`. Which work remains is tracked by `Task.status` and `Run`s—not by a status field on the `Document`.

When we say a **globally unique name**, we mean unique across all derived classes and instances **of that type** (for example all `Workflow`s, or all `WorkflowBranch` implementations). It does **not** mean unique across every class and object in the program: a `Stage.name` and a `WorkflowNode.name` may reuse the same string. `WorkflowNode.name` need only be unique **within its `Workflow`**. Cross-workflow / on-Document references use `f"{Workflow.name}.{WorkflowNode.name}"`; in-workflow references (`WorkflowBranch.next_node_names`, `entrypoints`) use the local `WorkflowNode.name` only.
...
| `workflow_node_name`  | `str \| None`                                                        | Yes         | Namespaced node id: `f"{Workflow.name}.{WorkflowNode.name}"`. Set on task submit or schedule—not database-generated. Used with `workflow_id` to load the node, its `Stage`, params, and branches. Also the key under which this task’s `StageResult` is stored on `Document.stage_results`. |
...
The `Executor` loads the `Stage` by resolving `Task.workflow_id` → `Workflow` → parse `Task.workflow_node_name` (`f"{Workflow.name}.{WorkflowNode.name}"`) → `WorkflowNode` → `stage_name`, then calls `process` with `WorkflowNode.stage_params` and that same namespaced `workflow_node_name`.

Inside `process`, the stage loads or creates the `Document` from the `Task` (`document_id`, `url`, `source`, …). It may read the **latest** prior `StageResult`s on that document (keyed by namespaced node id `f"{Workflow.name}.{WorkflowNode.name}"`, not by bare `Stage.name` or bare `WorkflowNode.name`).
...
**Persisting results.** The `Stage` itself stores its `StageResult` on the `Document` under the namespaced key `workflow_node_name` (`f"{Workflow.name}.{WorkflowNode.name}"`). That way the stage can merge the new result with a previous `StageResult` already present for the same node (for example on retry). The `Executor` still stores the returned `StageResult` on the `Task` for routing.

**Previous stage results.** The `Stage` can read the results of upstream stages that it depends on from the `Document.stage_results` dict. The keys under which the results of upstream stages are saved are namespaced (`f"{Workflow.name}.{WorkflowNode.name}"`) and must be passed through the `params` argument of `Stage.process()`.
...
class Stage(ABC, metaclass=StageMeta):
    ...
    async def process(
        ...
        May store the keys of upstream stages in `Document.stage_results`.
        ``workflow_node_name`` is the namespaced node id
        ``f"{Workflow.name}.{WorkflowNode.name}"``—the key under which this
        stage reads/writes ``Document.stage_results``.

        Load/create the ``Document`` from ``task`` as needed.
        Persist (and optionally merge) this run's ``StageResult`` on the
        document under ``workflow_node_name`` using an atomic partial update.
...
Each `WorkflowNode.name` must be unique **within that `Workflow` only** (not globally). Names are assigned when the pipeline is constructed, so `WorkflowBranch`es can list allowed next nodes before anything is saved to MongoDB. Do not use random or database-generated node ids.

**Namespaced references.** Wherever a node is referenced _outside_ the workflow graph itself—on `Task.workflow_node_name`, as a `Document.stage_results` key, or as the `workflow_node_name` argument to `Stage.process`—use the **namespaced** id:

```python
f"{Workflow.name}.{WorkflowNode.name}"
```

That namespaces nodes by workflow so two workflows may both have a node named `"extract"` without colliding on a shared `Document`.

**In-workflow references.** Inside a `Workflow`, `WorkflowBranch.next_node_names` and `Workflow.entrypoints` must use the **local** `WorkflowNode.name` only—not the namespaced form. Branches live inside a known workflow, so the prefix is redundant and must not be stored on the branch.

The same `stage_name` may appear on more than one node (different local node names, possibly different params and branches).

**In-memory registry.** After a `Workflow` is loaded, it builds a map from local `node.name` → `WorkflowNode` for O(1) lookup. Resolving `Task.workflow_node_name` strips or matches the `f"{Workflow.name}."` prefix, then looks up the local node name in that registry.
...
class Workflow(BeanieDocument):
    ...
    # local WorkflowNode.name values that start the workflow on submit

    def get_node(self, name: str) -> WorkflowNode:
        """Lookup by local or namespaced node name via the in-memory registry."""
        ...

    def namespaced_node_name(self, node_name: str) -> str:
        """Return f\"{self.name}.{node_name}\" for Task / Document / Stage.process refs."""
        ...
    ...
    async def submit(self, task: Task) -> Run:
        ...
        - For each name in entrypoints, bind a Task to that WorkflowNode
          (workflow_id; workflow_node_name = f\"{Workflow.name}.{node.name}\";
...
| ------------- | -------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `id`          | `ID`                 | Yes, unique | Globally unique id. Generated by MongoDB.                                                                                        |
| `name`        | `str`                | Yes, unique | Globally unique among `Workflow`s. Must not contain dots. How users identify and start this plan; prefix in namespaced node ids. |
| `nodes`       | `list[WorkflowNode]` | No          | All nodes. Order does not matter. Connections come from `WorkflowNode.branches`.                                                 |
| `entrypoints` | `list[str]`          | No          | Local `WorkflowNode.name`s to run when a `Task` is submitted. Must each exist in `nodes`. Namespaced on submit.                  |
...
| Field          | Type                   | Description                                                                                                                                                                                                                                         |
| -------------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `name`         | `str`                  | Unique within this `Workflow` only. Must not contain dots. `WorkflowBranch.next_node_names` and `entrypoints` use this local name. `Task.workflow_node_name`, `Document.stage_results`, and `Stage.process` use `f"{Workflow.name}.{name}"`.        |
| `stage_name`   | `str`                  | Registered `Stage.name` to run.                                                                                                                                                                                                                     |
| `stage_params` | `dict[str, Any]`       | Static parameters passed to `Stage.process` as `params`. When a `Stage` needs results of previous stages, pass the **namespaced** keys (`f"{Workflow.name}.{WorkflowNode.name}"`) under which those results are stored in `Document.stage_results`. |
...
When a `WorkflowBranch` refers to nodes (especially `next_node_names`), it must use the local `WorkflowNode.name`—**not** the namespaced `f"{Workflow.name}.{WorkflowNode.name}"`. The executor namespaces those names when writing `Task.workflow_node_name`.
...
class WorkflowBranch(ABC, BaseModel, metaclass=WorkflowBranchMeta):
    ...
    # allowed successors: local WorkflowNode.name only (not namespaced)
    params: dict[str, Any] = {}
    ...
    async def route(
        ...
        ``next_node_names`` (local names, resolved from the ``Workflow``
        registry). ``params`` are this branch's ``params`` field.

        Each returned ``Task`` must set ``workflow_id``,
        ``workflow_node_name`` to the *namespaced* id
        ``f"{Workflow.name}.{node.name}"`` for a node whose local ``name``
        is in ``next_node_names``, plus ``stage_name``, ``run_id``,
        ``parent_task_id``, etc.
        """
        ...
```
...
| `next_node_names` | `list[str]`      | Allowed successor **local** `WorkflowNode.name`s (not namespaced). Every produced `Task` must target one of these nodes; `Task.workflow_node_name` is set to `f"{Workflow.name}.{node.name}"`. |
...
1. After a successful stage, `Executor` resolves `next_nodes` from local `next_node_names` via the workflow registry and awaits `route(result, current_node, next_nodes, params)`.
2. Each produced `Task` must set `workflow_id`, `workflow_node_name = f"{Workflow.name}.{node.name}"` (local `node.name` ∈ `next_node_names`), `stage_name` (from that next node), `run_id`, `parent_task_id`, etc.
3. After the branch returns, `Executor` **verifies** that every produced `Task.workflow_node_name` equals `f"{Workflow.name}.{n}"` for some `n` in this branch’s `next_node_names`. Invalid targets are rejected.
...
When a `Task` completes, resolve the current `WorkflowNode` from the `Task` itself (`workflow_id` → `Workflow`, then `workflow_node_name` = `f"{Workflow.name}.{WorkflowNode.name}"` → registry). There is no need to go through the `Run` for that lookup. Then:
...
Query all `Task`s where `run_id = self.id` and wait until none remain in `CREATED`, `SCHEDULED`, `RUNNING`, or `RETRYING`. Entrypoint tasks are those whose `workflow_node_name` equals `f"{Workflow.name}.{ep}"` for some local name `ep` in `Workflow.entrypoints` and which don't have a parent task: `Task.parent_task_id is None`.
...
1. **New tasks** for stages that still have free capacity. When capacity for a `stage_name` is below its limit, the executor tries to claim another eligible task with that `stage_name` (atomic `findOneAndUpdate` as above) and starts it as an `asyncio` task.
...
`Document` is the durable product of the pipeline. Each finished `Stage` writes its `StageResult` onto the document under the namespaced key `f"{Workflow.name}.{WorkflowNode.name}"`. A `Stage` can also update individual fields of a `Document` as they are being computed, like `title`.

A document does **not** track its own completion or pipeline status: we do not know how many workflow runs a document needs. What to run next comes from `Task.status`; workflow progress comes from `Run`s.
...
| `stage_results` | `dict[str, list[StageResult]]` | No          | Results keyed by namespaced node id **`f"{Workflow.name}.{WorkflowNode.name}"`**, not by bare `Stage.name` or bare `WorkflowNode.name`, so the same stage used twice (or the same local node name in two workflows) keeps separate results. Lists allow history; a `Stage` reads the latest value per key. |
